...
public class GenerateTerrain : MonoBehaviour
{
    ...
    private void Generate()
    {
        if(RandomSeed)
        {
            Seed = Random.Range(-15000000, 15000000);
        }
        ...
        MeshFilter blockMesh = Instantiate(new GameObject(), Vector3.zero, Quaternion.identity).GetComponent<MeshFilter>();//create a unit cube and store the mesh from it
            ...
            for (int y = 0; y < chunkSize; y++)
            {
                for (int z = 0; z < chunkSize; z++)
                {
                    ...
                    {//is noise value above the threshold for placing a block?
                        ...
                        cubes.Add($"{MatrixExtractor.ExtractTranslationFromMatrix(blockMesh.transform.localToWorldMatrix)}");
                    }
        ...
        {
            File.AppendAllText(@"E:\test.txt", s + System.Environment.NewLine);
        }
        cubes.Clear();
